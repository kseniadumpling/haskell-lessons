...
{-
    Рекурсия:
    рассмотрим построение факториала
    1. fact = λr.λn.if n=0 then 1 else [(*) n (r r (- n 1))]
    2. Неподвижная точка функции f(x) = x. 
        Предположим, у нас есть комбинатор неподвижной точки:
        Y g = g (Y g)
        Y fact = fact (Y fact) = [λr.λn (= n 0) 1 [* n(r(- n 1))]] (Y fact) =
            = (λn (= n 0) 1 (* n ([Y fact] (- n 1))))
    3. (Y fact) 3 = [λn.(= n 0) 1 [* n ((Y fact)(- n 1))]] 3 = 
        (= 3 0) 1 [* 3 (Y fact)(- 3 1)] = 
        [* 3 [(λn.(= n 0) 1 (* n ([Y fact](- n 1))) 2)] = 
        [* 3 [* 2 [(λn.(= n 0) 1 (* n ([Y fact](- n 1))) 1)]]] = 
        [* 3 [* 2 [* 1 ((Y fact) 0)]]] = //TODO: дописать
    4. Комбинатор рекурсии: 
        Y ≡ λg.[[λx.g(x x)][λx.g(x x)]]

        Доказательство: 
        Y f = (λg.[[λx.g(x x)][λx.g(x x)]]) f = [λx.f(x x)][λx.f(x x)] = 
        = f[(λx.f(x x))(λx.f(x x))] ≡ f [Y f]

        Примечание: комбинатор неподвижной точки не работает с типами
-}


{-
    Каррирование - чек ит плз
-}
